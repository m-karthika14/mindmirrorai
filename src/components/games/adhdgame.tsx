...
const StartScreen = ({ onStart }) => (
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="max-w-3xl w-full mx-auto p-8 rounded-2xl bg-gradient-to-br from-black/40 to-slate-900/60 border border-slate-800 backdrop-blur-sm">
    <div className="flex flex-col items-center gap-4">
      ...
      <p className="text-center text-slate-300 max-w-2xl">A quick, gamified executive function assessment. You will complete practice trials, then three timed blocks that measure task switching and working memory (2-back). Focus and follow the rules — they will change.</p>
...
const GameScreen = ({ onComplete }) => {
  ...
  const ruleText = useMemo(() => {
    if (currentBlock.rule === 'shape') return 'Rule: Sort by SHAPE — (Circle/Square = LEFT, Triangle/Star = RIGHT)';
    if (currentBlock.rule === 'color') return 'Rule: Sort by COLOR — (Red/Blue = LEFT, Green/Yellow = RIGHT)';
    return 'Rule: 2-BACK — (Same shape as 2 trials ago = LEFT, otherwise = RIGHT)';
  }, [currentBlock.rule]);
